...
class linear():
    ...
    def derivative(self,xi, yi):
        self.gradient[0]=-2*(yi-  (self.w[0]+  self.w[1] * xi))
        self.gradient[1]=-2*(yi - (self.w[0] + self.w[1] * xi))*xi
...
def main():
    ...
    print("2017 error for 2017 linear model: " + str(g.calculateError()))
    ...
    for i in range(len(f.x)):
        error += (f.y[i] - (result[1]*f.x[i] + result[0])) ** 2
    print("2018 error for 2017 linear model: " + str(error))
